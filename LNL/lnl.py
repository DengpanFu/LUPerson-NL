...
parser.add_argument('--proto_m', type=float, default=0.999, 
                    help='proto momentum of updating prototype (default: 0.999)')
parser.add_argument('--alpha', type=float, default=0.5, 
                    help='weight to combine model prediction and prototype prediction')
parser.add_argument('--pseudo_th', type=float, default=0.8, 
                    help='threshold for pseudo labels')
parser.add_argument('--start_clean_epoch', type=int, default=11, 
                    help='epoch to start noise cleaning')
parser.add_argument('--start_supcont_epoch', type=int, default=12, 
                    help='epoch to start supcont')
parser.add_argument('--cls_dim', type=int, default=None)

parser.add_argument('--w_inst', type=float, default=1, 
                    help='weight for instance contrastive loss')
parser.add_argument('--w_proto', type=float, default=1, 
                    help='weight for prototype contrastive loss')

parser.add_argument('--cos', default=0, type=int, 
                    help='use cosine lr schedule')
parser.add_argument('--aug_type', default='ori-cj+sre', type=str)
parser.add_argument('--mean_type', default='lmdb_200_20', type=str)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Data loading code
    weak_transformer   = get_reid_train_transformer(args.mean_type, with_re=False)
    strong_transformer = get_lup_transformer(args.aug_type, args.mean_type, two_crop=False)
    val_transformer   = get_reid_test_transformer(args.mean_type)

    if args.rank == 0:
        print(f"Training Transformers: ")
        print(f"Weak: {str(weak_transformer)}")
        print(f"Strong: {str(strong_transformer)}")
        print(f"Testing  Transformers: \n{str(val_transformer)} \n")

    train_dataset = PersonDatasetFM(args.data_path, args.info_path, 
                                    weak_transformer, strong_transformer)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    losses = AverageMeter('L', ':.4f')
    losses_cls = AverageMeter('LC', ':.3f')
    losses_inst = AverageMeter('LI', ':.3f')
    losses_proto = AverageMeter('LP', ':.3f')
    top1_inst = AverageMeter('IAc@1', ':.2f')
    top5_inst = AverageMeter('IAc@5', ':.2f')
    top1_cls  = AverageMeter('CAc@1', ':.2f')
    top5_cls  = AverageMeter('CAc@5', ':.2f')

    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses, losses_cls, losses_inst, losses_proto, \
         top1_inst, top5_inst, top1_cls, top5_cls],
    ...
    is_supcont = epoch >= args.start_supcont_epoch
    ...
    for i, (im_q, im_k, labels) in enumerate(train_loader):
        ...
        if args.mix:
            with autocast():
                cls_out, target, loss_inst, logits_proto, accs_inst = \
                            model(im_q=im_q, im_k=im_k, target=labels, 
                                  is_proto=is_proto, is_clean=is_clean, 
                                  is_supcont=is_supcont, topk=(1, 5))
                loss_cls = criterion(cls_out, target)
                if is_proto:
                    loss_proto = criterion(logits_proto, target)
                else:
                    loss_proto = 0
                loss = loss_cls + args.w_inst * loss_inst + args.w_proto * loss_proto
        else:
            cls_out, target, loss_inst, logits_proto, accs_inst = \
                        model(im_q=im_q, im_k=im_k, target=labels, 
                              is_proto=is_proto, is_clean=is_clean, 
        ...
        losses_cls.update(loss_cls.item(), im_q.size(0))
        losses_inst.update(loss_inst.item(), im_q.size(0))
        if is_proto:
            losses_proto.update(loss_proto.item(), im_q.size(0))
        
        if target.size(0) >= 1:
            acc1, acc5 = accuracy(cls_out, target, topk=(1, 5))
            top1_cls.update(acc1[0].item(), target.size(0))
            top5_cls.update(acc5[0].item(), target.size(0))

        if not accs_inst is None:
            top1_inst.update(accs_inst[0].item(), im_q.size(0))
            top5_inst.update(accs_inst[1].item(), im_q.size(0))
    ...
    return losses.avg, top1_inst.avg
