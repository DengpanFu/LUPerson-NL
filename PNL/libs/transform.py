...
STAT_DICT = {'lmdb_300_30': {'mean': [0.3463, 0.3077, 0.3120], 'std': [0.2648, 0.2512, 0.2492]}, 
             'lmdb_200_20': {'mean': [0.3452, 0.3070, 0.3114], 'std': [0.2633, 0.2500, 0.2480]}, 
             'lmdb_100_10': {'mean': [0.3446, 0.3067, 0.3111], 'std': [0.2616, 0.2487, 0.2466]}, 
...
def get_reid_train_transformer(mean_type='lmdb_200_20', height=256, width=128, 
...
def get_reid_test_transformer(mean_type='lmdb_200_20', height=256, width=128):
    if not mean_type.lower() in STAT_DICT:
        raise TypeError('Unknown mean value status type: {}'.format(mean_type))
    mean = STAT_DICT[mean_type.lower()]['mean']
    std  = STAT_DICT[mean_type.lower()]['std']
...
def get_lup_transformer(aug_type='moco', mean_type='lup_v0', height=256, width=128, two_crop=True):
