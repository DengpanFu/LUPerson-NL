...
from libs.supcont_builder import SupCont
...
from libs.losses import SupConLoss
...
parser.add_argument('--has_bn', default=1, type=int)
...
def main():
    ...
    args.has_bn = True if args.has_bn >=1 else False
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = SupCont(models.__dict__[args.arch], dim=args.embed_dim, 
                    mlp=args.mlp, has_bn=args.has_bn)
    ...
    criterion = SupConLoss(T=args.T).cuda(args.gpu)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        is_save = args.multiprocessing_distributed and args.rank == 0
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        if args.mix:
            with autocast():
                output, target = model(im_q=images[0], im_k=images[1], label=labels)
                loss, accs = criterion(output, labels=target, topk=(1, 5))
        else:
            output, target = model(im_q=images[0], im_k=images[1], label=labels)
            loss, accs = criterion(output, labels=target, topk=(1, 5))
        ...
        top1.update(accs[0].item(), images[0].size(0))
        top5.update(accs[1].item(), images[0].size(0))
